...
export default function SocialChatPanel({ open, isLight, onClose }: SocialChatPanelProps) {
  const { t } = useT();
  ...
  useEffect(() => {
    ...
    const setupSubscription = async () => {
      ...
      // Query last 60 messages ordered by timestamp
      ...
      unsubscribe = onSnapshot(q, (snapshot) => {
        ...
        snapshot.forEach((doc) => {
          ...
          loadedMsgs.push({
            id: doc.id,
            uid: data.uid || '',
            displayName: data.displayName || 'Anonymous Pilgrim',
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'messages'), {
        ...
        text: inputText.trim().substring(0, 300), // cap message length
